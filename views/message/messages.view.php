...
<?php if (isset($id) && $id > 0 && $id != $_SESSION['user_id']) {
		...
		<div class="">
			...
			<div class="shadow p-4 rounded
				...
				<p class="rtext align-self-end
					...
					<?=$chat['message']?>
					<small class="d-block">
						<?=ucwords(strftime('%d %b %Y %T', strtotime($chat['created_at'])));?>
					</small>
				</p>
				<?php } else { ?>
				...
				let fechData = function() {
					$.post("<?=ROOT;?>" + "/messages/getMessages", {
						...
						function(data, status) {
							if (data.chat_id) {
								$("#chatBox").append(
									`<p class="ltext border rounded p-2 mb-1">${data.message}<small class="d-block">${data.created_at}</small></p>`
								);
								if (data != "") scrollDown();
							}
						});
						...
						<a href="<?=ROOT?>/messages/<?=$conversation['user']['user_id']?>"
							...
							<div class="d-flex
								...
								<h3 class="fs-xs m-2">
									...
									<small>
										<?php if ($conversation['chat']['to_id'] == $_SESSION['user_id']) {
										    if ($conversation['chat']['opened'] == 0) {
										        echo '<b style="color:black;">'.$chat->lastChat($_SESSION['user_id'], $conversation['chat']['from_id']).'</b>';
										    } else {
										        echo $chat->lastChat($_SESSION['user_id'], $conversation['chat']['from_id']);
										    }
										} else {
										    echo G_YOU.' : '.$chat->lastChat($_SESSION['user_id'], $conversation['chat']['to_id']);
										} ?>
									</small>
								</h3>
							</div>
